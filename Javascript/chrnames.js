import { CHRNAME } from '../Sites/Characters'
...
fetch('../../JSON/pagination.json')  // https://realcraymotherlol.github.io/JSON/paginationbowtop.json //
    .then(response => response.json())
    .then(content => {
        const currentPageData = content.CHRNAME.ID.Currentpage[pageNumber.toString()];
        ...
        artboxes.forEach((box, index) => {
            if (index < itemsPerPage) {
                ...
                if (currentPageData.hasOwnProperty(imgboxKey)) {
                    ...
                    box.style.background = `url(${backgroundImage})` + `${backgroundPosition}` + '/' + `${backgroundSize}` + "no-repeat";
