...
async function fetchOSInfo() {
  try {
    const response = await fetch('https://realcraymotherlol.github.io/JSON/MACSYSTEM.json');
    if (!response.ok) {
...
function fullscreen() {
    if (maxminbtn.getAttribute('aria-label') == 'Maximize') {
      elem.requestFullscreen();
      maxminbtn.setAttribute('aria-label', 'Restore');
    }
    else {
      document.exitFullscreen();
      maxminbtn.setAttribute('aria-label', 'Maximize');
    }
  }
